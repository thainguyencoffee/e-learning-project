...
class BrowseCourseControllerTests {
    ...
    private PublishedCourseDTO publishedCourseDTO;

    @BeforeEach
    public void setUp() {
        course = TestFactory.createDefaultCourse();
        publishedCourseDTO = new PublishedCourseDTO(
                course.getId(),
                course.getTitle(),
    ...
    void getAllPublishedCourses_ShouldReturnPageOfPublishedCourses() throws Exception {
        List<PublishedCourseDTO> courseList = List.of(publishedCourseDTO);
        Mockito.when(courseQueryService.findAllPublishedCoursesDTO(any(Pageable.class))).thenReturn(courseList);
    ...
    void getAllPublishedCourses_ShouldReturnEmptyPage_WhenNoPublishedCoursesExist() throws Exception {
        Mockito.when(courseQueryService.findAllPublishedCoursesDTO(any(Pageable.class))).thenReturn(List.of());
    ...
    void getPublishedCourseById_ShouldReturnPublishedCourse() throws Exception {
        Mockito.when(courseQueryService.findCoursePublishedById(1L)).thenReturn(publishedCourseDTO);
    ...
    void getPublishedCourseById_ShouldReturn404_WhenCourseNotFound() throws Exception {
        Mockito.when(courseQueryService.findCoursePublishedById(1L)).thenThrow(ResourceNotFoundException.class);
